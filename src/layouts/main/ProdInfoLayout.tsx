    ...
    <div className="flex flex-col justify-start gap-y-6 sm:mt-0 px-2 sm:px-0">
      ...
      <div className="flex flex-col gap-y-3">
        <span className="font-josefin">
          Color <span className="font-lexend text-sm">{selectedColorName ? ': ' : ''} {selectedColorName}</span>
        </span>
        <ColorRender
          items={product.items}
      ...
      <div>
        ...
        <span className="font-lexend">{selectedAvailableQty ? ': ' : ''}{selectedAvailableQty ?? ""}</span>
      </div>
